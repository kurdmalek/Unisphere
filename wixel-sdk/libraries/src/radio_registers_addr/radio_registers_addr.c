...
void radioRegistersInitAddr()
{
    ...
    // Packet control settings - this is where I'm doing mods to determine if h/w packet addressing works ok.
    PKTCTRL1 = 0x05;  // this one needs to change slightly, it was 0x04, we need to enable packet address checking here
	ADDR = 0x23;      // arbitrary address, the transmitter doesn't care, this is only for the RX, lets hardcode it for now, just for testing
    PKTCTRL0 = 0x45;  // Enable data whitening, CRC, and variable length packets.
	
}

BIT radioCrcPassedAddr()
{
    return (LQI & 0x80) ? 1 : 0;
}
